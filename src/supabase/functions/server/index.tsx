...
app.get("/make-server-04dd31ce/health", (c) => {
  return c.json({ status: "ok" });
});

// Test endpoint to verify server is working
app.get("/make-server-04dd31ce/test", (c) => {
  return c.json({ 
    message: "Server is working",
    timestamp: new Date().toISOString(),
    endpoints: {
      participants_public: "/make-server-04dd31ce/participants/public",
      participant_manual: "/make-server-04dd31ce/participant/manual"
    }
  });
});

// Register a new participant
app.post("/make-server-04dd31ce/register", async (c) => {
  try {
...
app.get("/make-server-04dd31ce/participants", requireAuth, async (c) => {
  try {
...
app.get("/make-server-04dd31ce/participants/public", async (c) => {
  try {
...
app.post("/make-server-04dd31ce/participant/manual", requireAuth, async (c) => {
...
app.get("/make-server-04dd31ce/participant/:id", async (c) => {
  try {
...
app.delete("/make-server-04dd31ce/participant/:id", requireAuth, async (c) => {
  try {
    const id = c.req.param('id');
...
app.put("/make-server-04dd31ce/participant/:id", requireAuth, async (c) => {
  try {
    const id = c.req.param('id');
...
app.post("/make-server-04dd31ce/participants/bulk", requireAuth, async (c) => {
  try {
...
// Record attendance
app.post("/make-server-04dd31ce/attendance", async (c) => {
  try {
...
// Admin signup
app.post("/make-server-04dd31ce/admin/signup", async (c) => {
  try {
...
// Get agenda items
app.get("/make-server-04dd31ce/agenda", async (c) => {
  try {
...
app.post("/make-server-04dd31ce/agenda", requireAuth, async (c) => {
  try {
    const body = await c.req.json();
...
app.put("/make-server-04dd31ce/agenda/:id", requireAuth, async (c) => {
  try {
    const id = c.req.param('id');
    const body = await c.req.json();
    const { title, description, startTime, endTime, location } = body;
...
app.delete("/make-server-04dd31ce/agenda/:id", requireAuth, async (c) => {
  try {
    const id = c.req.param('id');
